...
export function RuntimeControlClient() {
    ...
    const controlClassName = "w-max! h-10! sm:h-12! md:h-10! xl:h-12! ring-2! ring-white/2!"
    ...
    return (
        <>
            <div
                className={cn(
                    "pointer-events-none absolute h-40 w-[80%] -translate-x-1/2 -translate-y-1/2 rounded-full blur-2xl transition-[top,left,background-color] duration-500",
                    runtimeStyles[runtimeType]
                )}
            />
            <div
                className={cn(
                    "pointer-events-none absolute h-40 w-[80%] -translate-x-1/2 -translate-y-1/2 rounded-full blur-2xl transition-[top,left,background-color] duration-500",
                    executionStyles[executionType]
                )}
            />
