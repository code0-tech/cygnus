...
export function BuyMenu({ price, priceHeading, priceCaption, subscribeHref, subscribeLabel }: BuyMenuProps) {
    ...
    return createPortal(
        <div className="fixed bottom-4 left-1/2 z-50 grid w-[calc(100vw-2.5rem)] max-w-92 -translate-x-1/2 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-3xl border border-white/10 bg-primary/30 shadow-[0_8px_32px_0_rgba(0,0,0,0.45),0_0_0_1px_rgba(255,255,255,0.04)] backdrop-blur-md sm:bottom-6 sm:w-auto sm:max-w-none sm:grid-cols-[auto_auto] sm:gap-6 py-2.5 px-4 sm:py-2">
            <div className="min-w-0">
                <span className="text-sm font-semibold tracking-wide text-tertiary">{priceHeading}</span>
            ...
            <HapticButtonLink href={subscribeHref} variant="filled" className="justify-self-end bg-white/80! px-2.5! text-primary! hover:bg-white! font-semibold!  sm:px-4! sm:gap-3!">
                <IconShoppingCart size={18} stroke={2.2} className="-mx-1" />
                {subscribeLabel}
            </HapticButtonLink>
        </div>,
        document.body
