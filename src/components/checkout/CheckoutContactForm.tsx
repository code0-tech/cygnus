...
export function CheckoutContactForm({ customerSelect }: { customerSelect: ReactNode }) {
    const { content, customerType, guestEmail, continueNewCustomer, isLoading, stripeBillingAddress, stripeBillingAddressComplete, stripeEmail, stripeEmailComplete, setStripeBillingAddress, setStripeEmail } = useCheckoutFormState()
    ...
    return (
        <Elements stripe={stripePromise} options={{ appearance: stripeAppearance }}>
            <form className="w-full space-y-4" onSubmit={(event) => { event.preventDefault(); void continueNewCustomer() }}>
                {customerSelect}
                <EmailInput disabled={Boolean(guestEmail)} title={content.emailLabel} name="email" autoComplete="email" maxLength={254} placeholder={content.emailPlaceholder} value={stripeEmail ?? ""} onChange={(event) => setStripeEmail(event.currentTarget.value, emailValidation(event.currentTarget.value))} className="w-full!" />
