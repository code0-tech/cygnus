    ...
    <LandingContainer className="min-h-full pt-32 pb-8 overflow-visible">
      <div className={"relative flex flex-col gap-4 overflow-visible"}>
        <div className="flex max-w-xl flex-col gap-2">
          ...
          {footer.description && (
            <p className={"text-white/50 text-sm pb-4"}>{footer.description}</p>
          )}
        </div>

        <div
          className={
            "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 pb-16"
          }
        >
          {(footer.groups ?? []).map((group) => (
            <div
              className={"flex flex-col gap-1"}
              key={`${group.heading}-${group.id ?? "group"}`}
            >
              ...
              {(group.items ?? []).map((item) => (
                ...
                >
                  <p
                    className={
                      "text-white/75 hover:text-white hover:underline underline-offset-2 transition-colors"
                    }
                  >
                    {item.label}
                  </p>
            ...
            <div className="flex gap-2 flex-row items-center md:gap-4">
              ...
              <div className="flex items-center gap-4">
                {(footer.socialLinks ?? []).map((socialLink) => {
                  const Icon = socialLink.platform
