            ...
            {items.map((subItem, index) => (
                ...
                >
                    <Link
                        ...
                        className="group flex h-14 items-center gap-2 rounded-xl p-2 hover:bg-white/10 cursor-pointer">
                        <div className="ring ring-transparent group-hover:ring-white/10 rounded-lg bg-white/10 border border-white/20 p-1 text-gray-400 group-hover:text-white">
                            {subItem.icon}
                        </div>
                        <div className="flex min-w-0 flex-col">
                            <p className="truncate text-white font-semibold tracking-wide">{subItem.title}</p>
                            <p className="truncate text-white/75 text-xs">{subItem.description}</p>
                        </div>
                    </Link>
                </div>
            ))}
