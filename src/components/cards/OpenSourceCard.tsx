...
export const OpenSourceCard: React.FC = () => {
    return (
        <div className={"relative flex flex-col justify-between overflow-hidden gap-4 p-4 h-[420px] col-span-2 row-span-2 bg-white/1 rounded-xl border border-white/10"}>
            <FlickeringGrid
                className="relative inset-0 z-0 [mask-image:radial-gradient(240px_circle_at_center,white,transparent)]"
                squareSize={6}
                gridGap={4}
                color="#70ffb2"
                maxOpacity={0.5}
            ...
            <div className={"absolute top-24 left-10 w-4/5"}>
                <MergeCard />
            </div>
        ...
        <div className="relative w-full h-32 overflow-hidden">
            <AnimatePresence mode="wait">
                {currentStep >= 0 && currentStep !== 3 && (
                    <motion.div
                        key={"commit"}
                        className={cn("absolute inset-0 h-max p-2 bg-primary rounded-xl shadow-[0_-12px_24px_-10px_rgba(0,0,0,0.5)] border border-white/10 z-[11]")}
                    ...
                    >
                        <div className={"h-full flex items-center gap-1"}>
                            <IconGitCommit size={30} className={"text-[#575562] mr-1"}/>
                ...
                {currentStep >= 1 && currentStep !== 3 && (
                    <motion.div
                        key={"pr"}
                        className={"absolute inset-0 h-max p-2 bg-primary rounded-xl shadow-[0_-12px_24px_-10px_rgba(0,0,0,0.5)] border border-white/10 z-[12]"}
                        initial={{ y: 120, opacity: 0, scale: 0.8 }}
                    ...
                    >
                        <div className={"h-full flex items-center gap-1"}>
                            <IconGitPullRequest size={30} className={"text-[#575562] mr-1"}/>
                            <p className={"text-white/75 font-mono"}>Marius</p>
                ...
                {currentStep === 2 && (
                    ...
                    >
                        <div className={"h-full flex items-center gap-1"}>
                            <IconGitMerge size={30} className={"text-[#575562] mr-1"}/>
