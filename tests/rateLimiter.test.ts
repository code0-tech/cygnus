...
test("blocks requests after the configured fixed-window limit", () => {
    ...
    assert.deepEqual(limiter.consume("login:key", policy), { allowed: true, limit: 2, remaining: 1, resetSeconds: 60 })
    assert.deepEqual(limiter.consume("login:key", policy), { allowed: true, limit: 2, remaining: 0, resetSeconds: 60 })
    assert.deepEqual(limiter.consume("login:key", policy), { allowed: false, limit: 2, remaining: 0, resetSeconds: 60 })
